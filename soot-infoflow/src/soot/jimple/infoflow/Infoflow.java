...
public class Infoflow extends AbstractInfoflow {
    ...
    private void runAnalysis(final ISourceSinkManager sourcesSinks, final Set<String> additionalSeeds) {
        ...
        try {
            ...
            // todo
            String analysisAlgorithm = "cha";
            if (analysisAlgorithm.equals("cha")) {
...
//			}
            ...
            if (config.getEnableReflection()) {//false
                releaseCallgraph();
                constructCallgraph();
            }

            //CallgraphAlgorithm：AutomaticSelection, CHA, VTA, RTA, SPARK, GEOM, OnDemand
            //AutomaticSelection
            //OnDemand
...
//				System.out.println("backwardSolver = " + backwardSolver);//backwardSolver = soot.jimple.infoflow.solver.fastSolver.InfoflowSolver@608c7b5b
                ...
                try {
                    ...
                    for (SootMethod sm : getMethodsForSeeds(iCfg)) {
                        try {
//							System.out.println(scanMethodForSourcesSinks(sourcesSinks, forwardProblem, sm));
//                            System.out.println(sourcesSinks);
                            sinkCount += scanMethodForSourcesSinks(sourcesSinks, forwardProblem, sm);
//							if(scanMethodForSourcesSinks(sourcesSinks, forwardProblem, sm) > 0){
...
//							}
                    ...
                    // System.out.println("test: sinkCount = " + sinkCount);//4

                    // We optionally also allow additional seeds to be specified
//					System.out.println(additionalSeeds);//null
                    ...
                    logger.info("Source lookup done, found {} sources and {} sinks.",
                            forwardProblem.getInitialSeeds().size(), sinkCount);//6、4
...
//					System.out.println(performanceData.getTaintPropagationSeconds());  //-1
...
//					System.out.println(executor.getActiveCount());//0
...
//					System.out.println("taintPropagationSeconds = " + taintPropagationSeconds);//0
                    performanceData.addTaintPropagationSeconds(taintPropagationSeconds);

                    // Print taint wrapper statistics
                    if (taintWrapper != null) {
                        logger.info("Taint wrapper hits: " + taintWrapper.getWrapperHits());//42
                        logger.info("Taint wrapper misses: " + taintWrapper.getWrapperMisses());//99
                    }
                    ...
                    //IFDS problem with 1055 forward and 39 backward edges solved in 0 seconds, processing 3 results...

                    // Update the statistics
                    {
                        ISolverTerminationReason reason = ((IMemoryBoundedSolver) forwardSolver).getTerminationReason();
//						System.out.println("test---reason: " + reason);//null
                        if (reason != null) {
                            if (reason instanceof OutOfMemoryReason)
                                results.setTerminationState(
                                        results.getTerminationState() | InfoflowResults.TERMINATION_DATA_FLOW_OOM);//2
                            else if (reason instanceof TimeoutReason)
                                results.setTerminationState(
                                        results.getTerminationState() | InfoflowResults.TERMINATION_DATA_FLOW_TIMEOUT);//1
                        }
                    }
                    ...
                    // now.
                    performanceData.updateMaxMemoryConsumption(getUsedMemory());
//					System.out.println("getUsedMemory() = " + getUsedMemory());//91、150、92、150、90
...
//					System.out.println(config.getIncrementalResultReporting());//false
                    if (config.getIncrementalResultReporting())
                        res = null;
                    iCfg.purge();
                    ...
                    performanceData.updateMaxMemoryConsumption(getUsedMemory());
                    logger.info(String.format("Memory consumption after cleanup: %d MB", getUsedMemory()));//49

//					System.out.println("test11");
//					System.out.println(config.getPathConfiguration().getPathReconstructionTimeout());//0
                    ...
                    if (config.getIncrementalResultReporting()) {
                        ...
                        InfoflowResults actualResults = builder.getResults();
                        this.results.addAll(actualResults);
                        ConcurrentHashMap<Pair<ResultSinkInfo, ResultSourceInfo>, List<List<Stmt>>> map = actualResults.getMapContainsList();
                    }
                ...
                logger.info(String.format("Memory consumption after path building: %d MB", getUsedMemory()));//46、50、46
            ...
            if (results == null || results.isEmpty())
                ...
                int count = 0;
                HashSet<PathInfo> pathInfos = new HashSet<>();
                HashSet<String> bothStartAndEndStmts = new HashSet<>();
                HashSet<String> endStmts = new HashSet<>();
                HashSet<PathInfo> removedPathInfos = new HashSet<>();
                for (ResultSinkInfo sink : results.getResults().keySet()) {
                    endStmts.add(sink.getStmt().toString());
                }
                for (ResultSinkInfo sink : results.getResults().keySet()) {
                    for (ResultSourceInfo source : results.getResults().get(sink)) {
                        ArrayList<ResultSourceInfo> paths = new ArrayList<>();
                        paths.add(source);
                        PathInfo pathInfo = new PathInfo(source, sink, paths);
                        pathInfos.add(pathInfo);
                        if (endStmts.contains(source.getStmt().toString())) {
                            bothStartAndEndStmts.add(source.getStmt().toString());
                            removedPathInfos.add(pathInfo);
                        }
                    }
                }

                for (PathInfo p1 : pathInfos) {
                    if (bothStartAndEndStmts.contains(p1.getEndStmt().getStmt().toString())) {
                        for (PathInfo p2 : pathInfos) {
                            if (bothStartAndEndStmts.contains(p2.getStartStmt().getStmt().toString())
                                    && p2.getStartStmt().getStmt().toString().equals(p1.getEndStmt().getStmt().toString())) {
                                p1.getPath().addAll(p2.getPath());
                                p1.setEndStmt(p2.getEndStmt());
                                break;
                            }
                        }
                    }
                }
                pathInfos.removeAll(removedPathInfos);
                for (PathInfo pathInfo : pathInfos) {
                    logger.info("\ton Path: ");
                    boolean flag = false;
                    for (ResultSourceInfo source : pathInfo.getPath()) {
                    ...
                    count++;
                    System.out.println("Path number：" + count);
                }
            }
            ...
            logger.error("Exception during data flow analysis", ex);//数据流分析期间出现异常
            if (throwExceptions)
                throw ex;
        }
    }
