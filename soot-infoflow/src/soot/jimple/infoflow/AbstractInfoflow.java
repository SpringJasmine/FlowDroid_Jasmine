...
public abstract class AbstractInfoflow implements IInfoflow {
    ...
    protected void constructCallgraph() {
        ...
        for (PreAnalysisHandler tr : preProcessors)
            tr.onBeforeCallgraphConstruction();//在构造调用图之前调用此方法
        ...
        try {
            ...
            // delete: FEBS-cloud-auth:QueryRequest
            e.printStackTrace();
        }

        // CallGraph callGraph = Scene.v().getCallGraph();
        // QueueReader iterator = callGraph.listener();

        // while (iterator.hasNext()) {
        //     Edge edge = (Edge) iterator.next();
        //     // JSONObject methodsObject = new JSONObject();
		//  	if (edge.getSrc().method().toString().contains("com.favorites.")
		//  			|| edge.getTgt().method().toString().contains("com.favorites.")
		//  			|| edge.getTgt().method().toString().contains("synthetic.method")
		//  			|| edge.getSrc().method().toString().contains("synthetic.method") ) {
        //         System.out.println("srcMethod: " + edge.getSrc().method().toString());
        //         System.out.println("tgtMethod" + edge.getTgt().method().toString());
        //     }
        // }
